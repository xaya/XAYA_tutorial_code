...
namespace BitcoinLib.Services
{
    ...
    public partial class CoinService : ICoinService
    {
        ...
        public GetShowNameResponse ShowName(string name)
        {
            ...
            // Initialise all fields as though the name does not exist.
            response.name = "Does not exist.";
            response.name_encoding = "Does not exist.";
            response.name_error = "Does not exist.";
            response.value = "Does not exist.";
            response.value_encoding = "Does not exist.";
            response.txid = "Does not exist.";
            response.vout = -1;
        ...
        //public string NamePending(string name)
        //{
        //    return _rpcConnector.MakeRequest<string>(RpcMethods.name_pending);
        //}

        // RYAN - I think these should be in the wrapper, and not here in the RPC code. 
        // They're from libxayagame, and not from the XAYA daemon. We're a bit muddy this way.
        public void WaitForChange()
        {
            _rpcConnector.MakeRequest<string>(RpcMethods.waitforchange);
        }

        public GetGameStateResponse GetCurrentState()
        {
            return _rpcConnector.MakeRequest<GetGameStateResponse>(RpcMethods.getcurrentstate);
        }
        ...
        private static bool IsValidName(string name, string XAYAnamespace = "p/")
        {
            bool result = false;
