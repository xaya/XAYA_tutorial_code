    ...
    public partial class CoinService
    {
        ...
        public class CoinParameters
        {
                ...
                short rpcRequestTimeoutInSeconds)
            {
                ...
                else if (coinService is XAYAService)
                {
                    if (!IgnoreConfigFiles)
                    {
                        DaemonUrl = ConfigurationManager.AppSettings.Get("XAYA_DaemonUrl");
                        DaemonUrlTestnet = ConfigurationManager.AppSettings.Get("XAYA_DaemonUrl_Testnet");
                        RpcUsername = ConfigurationManager.AppSettings.Get("XAYA_RpcUsername");
                        RpcPassword = ConfigurationManager.AppSettings.Get("XAYA_RpcPassword");
                        WalletPassword = ConfigurationManager.AppSettings.Get("XAYA_WalletPassword");
                    }
                    ...
                    TotalCoinSupplyInCoins = 84000000;
                    EstimatedBlockGenerationTimeInMinutes = 0.66;
                    BlocksHighestPriorityTransactionsReservedSizeInBytes = 16000;
                    BlockMaximumSizeInBytes = 250000;
